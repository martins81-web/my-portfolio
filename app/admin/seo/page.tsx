...
export default function AdminSeoPage() {
  ...
  useEffect(() => {
    async function load() {
      const r = await fetch("/api/admin/content?key=seo", { cache: "no-store" })
  ...
  async function save() {
    ...
    const r = await fetch("/api/admin/content", {
      ...
      body: JSON.stringify({ key: "seo", content: parsed, message: "Update seo content" }),
    })
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-6 py-12">
      <h1 className="text-2xl font-semibold">Edit seo</h1>
